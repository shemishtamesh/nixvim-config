{ utils, ... }:
{
  plugins.otter = {
    ...
    settings.buffers.preambles = 
      ...
      lua = [
        "---@diagnostic disable: duplicate-set-field, exp-in-action, inject-field, miss-name, miss-sep-in-table, miss-symbol, undefined-global, trailing-space, unreachable-code, unused-local, unknown-symbol, err-eq-as-assign, redefined-local, unexpect-symbol"
      ];
    };
  };
  extraConfigLua = /* lua */ ''
